...
export default class MapaApiarios extends Component {
    ...
    constructor(props) {  
        ...
        this.agregarTodosLosApicultores = this.agregarTodosLosApicultores.bind(this);
        this.agregarApicultores = this.agregarApicultores.bind(this);
        this.agregarColmenas = this.agregarColmenas.bind(this);
    ...
    this.setState({ ciudad_seleccionada : event.target.value }, () => {
      ...
      if ( this.state.ciudad_seleccionada != "" ) {
          ...
          if( this.state.ciudad_seleccionada == "Todos" ) {
             this.agregarTodosLosApicultores();
          }
          else {
            // Apicultores que tienen apiarios en dicha ciudad.
            this.agregarApicultores(this.state.ciudad_seleccionada);
            // Apiarios en dicha ciudad.
            this.agregarColmenas(this.state.ciudad_seleccionada);
          }
      }
    });
  }
  ...
  agregarTodosLosApicultores() {
    ...
    for( var i=0; i<apiarios_existentes.length; i++ ) {
      ...
      if( apicultores.includes(apiarios_existentes[i]['apicultores']['id'])  ) continue;

        apicultores.push(apiarios_existentes[i]['apicultores']['id']);

        this.createDefaultValueSelector(
          "selector-apicultor",  
          apiarios_existentes[i]['apicultores']['id'], 
          apiarios_existentes[i]['apicultores']['name'] + " " + apiarios_existentes[i]['apicultores']['lastname'],
          apiarios_existentes[i]['apicultores']['id']
        );
  ...
  agregarApicultores(ciudad) { 
    ...
    for( var i=0; i<apiarios_existentes.length; i++ ) {
      ...
      if( apiarios_existentes[i]['apiario']['localidad_chacra'] == ciudad ) {
  ...
  handleClickBuscar(event) {
      ...
      var apicultor = this.state.apicultor_seleccionado;      
      ...
      if ( apiario_checked ) {
        ...
        apiarios = this.getApiario(apiario_id);
          ...
          if( apicultor != "" ) {
              apiarios = this.getApiariosApicultor(apiarios,apicultor);
          }
  ...
  getApiariosApicultor(apiarios,apicultor) {

    var resultado = [];

    for( var i = 0; i < apiarios.length; i++ ) {
      
      if( apiarios[i]['apiario']['apicultor_id'] == apicultor ) resultado.push(apiarios[i]);
    }
        ...
        <div className="col-md-12">
          <div className="box box-success">
            <div className="box-header with-border">
              <h3 className="box-title"><strong>Estado Actual de los Apiarios en el VIRCH</strong></h3>
            ...
            {/* <div className="box box-info">
                <div className="box-header with-border ">
                  <h3 className="box-title">Detalle de los Apiarios Seleccionados</h3>  
                </div>
                <div className="box-body">
                  <div className="row">
                    <div className="col-md-12">  */}

                            {/* DataTable Tabla Colmenas Seleccionadas */}
                            {/* <div className="col-xs-12 col-md-12 col-lg-12" style={{marginTop:20}}>
                    ...
                    {/* ./col */}
                    {/* <Apiario apiario={[]} apiario_id={1} direccion_chacra="Chacra 1234" key={1} action={""}/>
                    <Apiario apiario={[]} apiario_id={2} direccion_chacra="Chacra 1234" key={2} action={""}/>
                    <Apiario apiario={[]} apiario_id={3} direccion_chacra="Chacra 1234" key={3} action={""}/>
                    <Apiario apiario={[]} apiario_id={4} direccion_chacra="Chacra 1234" key={4} action={""}/>
                    <Apiario apiario={[]} apiario_id={5} direccion_chacra="Chacra 1234" key={5} action={""}/>
                    <Apiario apiario={[]} apiario_id={6} direccion_chacra="Chacra 1234" key={6} action={""}/>
                    <Apiario apiario={[]} apiario_id={7} direccion_chacra="Chacra 1234" key={7} action={""}/> */}
                  {/* </div> */}
                  {/* ./row */}
                {/* </div> */}
                {/* ./box body */}
